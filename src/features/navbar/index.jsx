...
export default function Navbar() {
  ...
  const cartCompareAmount = useSelector((state) => state.cart.compare_amount);
  const cartTotalItems = useSelector((state) => state.cart.total_items);
  ...
  const ethPrice = useSelector((state) => state.crypto.eth_price);
  const ethereumPrice = ethPrice && ethPrice.ethereum.usd;
  const adaPrice = useSelector((state) => state.crypto.ada_price);
  const cardanoPrice = adaPrice && adaPrice.cardano.usd;

  // eslint-disable-next-line
  const [openI18, setOpenI18n] = useState(false);
  ...
  return (
    <header className="bg-white shadow-navbar border-b w-full z-30 lg:overflow-y-visible dark:border-dark-second dark:bg-dark-main">
      ...
      <DesktopNavbar
        ...
        cartCompareAmount={cartCompareAmount}
