...
export default function ImageGallery({ course, courseUUID, setHasChangesImages }) {
  const courseImages = course && course.images;
  const reduxImages = useSelector((state) => state.courses.images);

  const dispatch = useDispatch();

  const [imagesList, setImagesList] = useState(
    courseImages && courseImages.length !== 0 ? courseImages : reduxImages,
  );

  const [originalImages, setOriginalImages] = useState([]);
  useEffect(() => {
    setImagesList(courseImages && courseImages.length !== 0 ? courseImages : reduxImages);
    dispatch(
      onchangeCourseImage(courseImages && courseImages.length !== 0 ? courseImages : reduxImages),
    );
  ...
  const handleDrop = (acceptedFiles) => {
    ...
    Promise.all(newItems)
      .then((items) => {
        ...
        dispatch(onchangeCourseImage([...base64Images, ...items]));
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  async function handleImageDelete(imageId) {
    dispatch(deleteCourseImage(courseUUID, imageId));
  }
  ...
  useEffect(() => {
    if (base64Images.length > 0) dispatch(onchangeCourseImage(base64Images));
  }, [base64Images, dispatch]);

  useEffect(() => {
    dispatch(onchangeCourseImage(imagesList));
  }, [imagesList, dispatch]);
