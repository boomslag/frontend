...
export default function VideoGallery({ course, courseUUID, setHasChangesVideos, percentage }) {
  const courseVideos = course && course.videos;
  const reduxVideos = useSelector((state) => state.courses.videos);

  const dispatch = useDispatch();

  const [videosList, setVideosList] = useState(
    courseVideos && courseVideos.length !== 0 ? courseVideos : reduxVideos,
  );

  const [originalVideos, setOriginalVideos] = useState([]);
  useEffect(() => {
    setVideosList(courseVideos && courseVideos.length !== 0 ? courseVideos : reduxVideos);
    dispatch(
      onchangeCourseVideo(courseVideos && courseVideos.length !== 0 ? courseVideos : reduxVideos),
    );
  ...
  async function handleVideoDelete(item) {
    dispatch(deleteCourseVideo(courseUUID, item.id));
  }
  ...
  useEffect(() => {
    if (base64Videos.length > 0) dispatch(onchangeCourseVideo(base64Videos));
  }, [base64Videos, dispatch]);

  useEffect(() => {
    dispatch(onchangeCourseVideo(videosList));
  }, [videosList, dispatch]);
