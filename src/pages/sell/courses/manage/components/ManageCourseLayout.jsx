...
export default function ManageCourseLayout({
    ...
    <button
      onClick={async () => {
        if ((details && details.nft_address === '0') || details.nft_address === null) {
          ...
          await dispatch(updateCourseStatus(courseUUID, true));
          fetchCourse();
        }
      }}
      type="button"
  ...
  const onCreateNFT = async (e) => {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        await EditNFTAddress(courseUUID, res.data.results.contractAddress);
        await dispatch(updateCourseStatus(courseUUID, true));
        fetchCourse();
  ...
  if (!isAuthenticated && myUser.role !== 'seller' && myUser.id !== course.details.id) {
    router.push('/sell');
  }
                ...
                <Dialog.Panel className="relative flex w-full max-w-xs flex-1 flex-col dark:bg-dark-bg bg-white">
                  ...
                  <div className="h-0 flex-1 overflow-y-auto">
                    <nav className=" space-y-1 ">
                      <Sidebar history={history} course={course} courseUUID={courseUUID} />
                    </nav>
                  </div>
