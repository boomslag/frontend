...
export default function Goals() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    await SetCourseHandle(courseUUID, true, 'goals');

    const promises = [];

    if (hasChangesWhatLearnt) {
      promises.push(dispatch(updateCourseWhatlearnt(courseUUID, whatLearnt)));
    }

    if (hasChangesWhoIsFor) {
      promises.push(dispatch(updateCourseWhoIsFor(courseUUID, whoIsFor)));
    }

    if (hasChangesRequisite) {
      promises.push(dispatch(updateCourseRequisite(courseUUID, requisites)));
    }
