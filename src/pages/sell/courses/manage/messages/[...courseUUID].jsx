...
export default function Messages() {
  ...
  const handleSubmit = async () => {
    setLoading(true);
    await SetCourseHandle(courseUUID, true, 'messages');

    const promises = [];

    if (hasChangesWelcomeMessage) {
      promises.push(dispatch(updateCourseWelcomeMessage(courseUUID, welcomeMessage)));
    }
    if (hasChangesCongratsMessage) {
      promises.push(dispatch(updateCourseCongratsMessage(courseUUID, congratsMessage)));
    }
