...
export default function Settings() {
  ...
  async function handleDelete() {
    if (canDelete) {
      const res = await DeleteCourse(courseUUID);
      ToastSuccess(res);
      history.push('/sell/courses/');
    }
  }
    ...
    if (canPublish) {
      if (details.nft_address !== '0') {
        if (details.status === 'published') {
          dispatch(updateCourseStatus(courseUUID, false));
        } else {
          dispatch(updateCourseStatus(courseUUID, true));
        }
      }
      fetchCourse();
    }
